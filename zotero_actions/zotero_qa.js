...
//         - 为了避免一直把全量文本加入到prompt里，对文档建立一个搜索库，使用RAG技术
// - 问答记录应该不断追加到这个item下的某个note里
// - 自动问题池
//     - 哪些相关论文最值得阅读。
//        - 最好去搜我现有的库列表，排除已有的
//        - 去搜索学术网站，获得引用数来确定重要性
//        - 分类：前置的重要论文；类似课题的重要论文
...
class ZoteroLLMQA {
  ...
  async getContext() {
    ...
    let window_type =
      Zotero_Tabs.selectedID == "zotero-pane" ? "main" : "reader";
    if (window_type == "main") {
      ...
      const PDFViewerApplication =
        reader._iframeWindow.wrappedJSObject.PDFViewerApplication; // 获取 PDFViewerApplication 对象
  ...
  async getQuestionAnswer() {
    try {
      ...
      if (
        contextInfo.window_type == "main" &&
        contextInfo.selected_items.length == 0
      ) {
        // 搜索文献 是不是main就得搜索呢？
  ...
  async getPrompt(query, contextInfo) {
    let contextInfoStr = "";
    contextInfo["selected_items"] = contextInfo["selected_items"]
      .map((item) => getItemString(item))
      .join("\n\n");
    contextInfo["related_items"] = contextInfo["related_items"]
      .map((item) => getItemString(item))
      .join("\n\n");
