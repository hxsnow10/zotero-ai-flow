...
class ElasticsearchSearchTool:
    ...
    def __init__(self, es_host="http://localhost:9200", es_index="zotero_papers"):
        self.es_client = ElasticsearchClient(hosts=es_host, index_name=es_index)
        ...
        try:
            ...
            if vector:
                results = self.es_client.search(
                    query=query, vector=vector, size=size, filters=filters
                )
            else:
            ...
            if group_by_parent:
                # 按得分排序
                parent_items = sorted(
                    parent_docs.items(), key=lambda x: x[1]["max_score"], reverse=True
                )
...
def search_elasticsearch(
    ...
    """
    return good_read(
        es_client.search(query, vector=vector, size=size), "elasticsearch 搜索结果"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i, item in enumerate(results):
            ...
            print(
                f"作者: {', '.join([f'{a.get('firstName', '')} {a.get('lastName', '')}' for a in item['authors'] if 'lastName' in a])}"
            )
            print(f"日期: {item['date']}")
            if item["abstract"]:
                print(
                    f"摘要: {item['abstract'][:100]}..."
                    if len(item["abstract"]) > 100
                    else item["abstract"]
                )
    except Exception as e:
