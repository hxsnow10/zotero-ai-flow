...
class SemanticScholarAPI:
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: str = "https://api.semanticscholar.org/graph/v1"):
        ...
        if self.api_key:
            ...
            logger.warning("未提供API密钥，将受到更严格的速率限制，建议申请API密钥：https://www.semanticscholar.org/product/api")
            # 无API密钥时使用更长的请求间隔
        ...
        while self.retry_count <= self.max_retries:
            try:
                ...
                if response.status_code == 429 and retry_on_429:
                    ...
                    logger.warning(f"遇到速率限制，等待 {wait_time:.2f} 秒后重试 (尝试 {self.retry_count}/{self.max_retries})")
                    time.sleep(wait_time)
                    continue
    ...
    def search_papers(self, 
        ...
        try:
            ...
            if hasattr(e, 'response') and e.response and e.response.status_code == 429:
                logger.error("请考虑申请API密钥以获得更高的速率限制: https://www.semanticscholar.org/product/api#api-key-form")
                
            return [] if not return_raw else {"error": str(e)}
    
    def get_paper_details(self, 
                         paper_id: str, 
                         fields: str = "title,authors,abstract,citations,references,year,venue,url,tldr") -> Dict:
        """
        获取单篇论文的详细信息
    ...
    def get_author_details(self, 
                         author_id: str, 
                         fields: str = "name,paperCount,citationCount,hIndex") -> Dict:
        """
        获取作者详细信息
    ...
    def get_author_papers(self, 
                         author_id: str, 
                         limit: int = 10, 
                         fields: str = "title,year,venue,abstract") -> List[Dict]:
        """
        获取作者的论文列表
...
def main(args):
    ...
    # 获取作者详情
        ...
        if args.author_papers:
            ...
            papers = api.get_author_papers(args.author_id, limit=args.limit, fields=args.fields)
...
if __name__ == "__main__":                                                                                                                                                                                                                    
    ...
    parser.add_argument("-f", "--fields", default="title,authors,abstract,year,venue,url", type=str, help="返回字段，逗号分隔")
    parser.add_argument("-p", "--paper_id", default=None, type=str, help="论文ID")
    parser.add_argument("-pd", "--paper_details", action="store_true", help="获取论文详情")
    parser.add_argument("-a", "--author_id", default=None, type=str, help="作者ID")
    parser.add_argument("-ap", "--author_papers", action="store_true", help="获取作者论文")
    parser.add_argument("-k", "--api_key", default=None, type=str, help="Semantic Scholar API密钥")
