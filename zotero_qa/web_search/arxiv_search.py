...
def main(args):
    ...
    # 按查询搜索
        ...
        if args.output_json:
            paper_info_list = [
                arxiv_search.format_paper_info(paper) for paper in results
            ]
            with open(args.output_json, "w", encoding="utf-8") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--category", default=None, type=str, help="限制搜索的分类，如cs.AI"
    )
    parser.add_argument(
        "--sort-by",
        default="relevance",
        type=str,
        help="排序标准: relevance, lastupdate, submitted",
    )
    parser.add_argument(
        "--sort-order",
        default="descending",
        type=str,
        help="排序顺序: ascending, descending",
    )

    # 论文ID相关参数
    parser.add_argument(
        "-i", "--id", default=None, type=str, help="论文ID，如2303.08774"
    )

    # 下载相关参数
    parser.add_argument("-d", "--download", action="store_true", help="下载论文PDF")
    parser.add_argument(
        "-o", "--output-dir", default="downloads", type=str, help="下载PDF的目录"
    )

    # 输出相关参数
    parser.add_argument(
        "-j", "--output-json", default=None, type=str, help="保存结果到JSON文件"
    )
