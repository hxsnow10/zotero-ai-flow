...
class DocumentSplitter:
    ...
    def split_by_paragraph(
        text: str, min_paragraph_length: int = 50, merge_short: bool = True
    ) -> List[Dict[str, Any]]:
        """
        按段落进行切片
    ...
    def split_by_section(
        text: str, section_patterns: List[str] = None
    ) -> List[Dict[str, Any]]:
        """
        按章节标题进行切片
...
"""
    ...
    print("=== 按固定大小切片 ===")
    chunks = DocumentSplitter.split_text(
        sample_text, "chunk", chunk_size=200, overlap=50
    )
    for i, chunk in enumerate(chunks):
        print(
            f"Chunk {i+1} ({len(chunk['content'])} chars): {chunk['content'][:50]}..."
        )
    ...
    for i, chunk in enumerate(chunks):
        print(
            f"Paragraph {i+1} ({len(chunk['content'])} chars): {chunk['content'][:50]}..."
        )
