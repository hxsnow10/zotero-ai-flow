...
class ElasticsearchClient:
    ...
    def __init__(
        ...
        try:
            ...
            if self.recreate_index and self.es.indices.exists(index=self.index_name):
                # 谨慎处理删除
                logger.info(
                    f"Deleting existing index '{self.index_name}' for recreation..."
                )
            ...
            # 如果提供了自定义映射，立即创建新索引
            if (
                not self.es.indices.exists(index=self.index_name)
                and self.custom_mapping
            ):
                logger.warning(
                    f"Creating index '{self.index_name}' with custom mapping..."
                )
                self.create_index(mapping=self.custom_mapping)

        except Exception as e:
            raise ConnectionError(
                f"Cannot connect to Elasticsearch at {hosts}: {str(e)}"
            )
    ...
    def insert_document(
        ...
        try:
            return self.es.index(
                index=self.index_name, id=doc_id, document=doc_to_insert
            )
        except Exception as e:
    ...
    def search(
        ...
        if query and vector:
            ...
            try:
                ...
                body = {
                    ...
                    "post_filter": {"bool": {"must": [text_query]}},
                }
                ...
                logger.warning(
                    f"KNN query failed, falling back to script_score: {str(e)}"
                )
                # 回退到 script_score 查询
                body = {
                    "query": {
            ...
            try:
                ...
                if filter_conditions:
                    body["query"]["script_score"]["query"] = {
                        "bool": {"must": {"match_all": {}}, "filter": filter_conditions}
                    }
        ...
        try:
            ...
            return self.es.search(
                index=self.index_name, body=body, size=size, from_=from_
            )
        except Exception as e:
    ...
    def vector_search(
        self, vector: List[float], size: int = 10, filters: Optional[Dict] = None
    ) -> Dict:
        """
    ...
    def update_document(
        self, doc_id: str, updates: Dict[str, Any], vector: Optional[List[float]] = None
    ) -> Dict:
        """
        ...
        try:
            return self.es.update(
                index=self.index_name, id=doc_id, body={"doc": doc_updates}
            )
        except Exception as e:
...
def main(args):
    ...
    try:
        ...
        if args.vector_search and args.with_vector:
            ...
            search_results = es_client.vector_search(vector=query_vector, size=10)
        else:
            # 执行更简单的文本查询，不使用过滤器
        ...
        if len(search_results["hits"]["hits"]) > 0:
            for hit in search_results["hits"]["hits"]:
                ...
                if "vector" in source_copy:
                    source_copy["vector"] = (
                        f"[Vector with {len(source_copy['vector'])} dimensions]"
                    )

                logger.info(
                    f"Score: {hit['_score']}, Document: {source_copy.get('title', 'No title')}"
                )
            ...
            all_docs = es_client.es.search(
                index=args.index, body={"query": {"match_all": {}}}
            )
            ...
            for doc in all_docs["hits"]["hits"]:
                ...
                logger.info(
                    f"ID: {doc['_id']}, Title: {source_copy.get('title', 'No title')}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input_path", default=None, type=str, help="Path to input data file"
    )
    parser.add_argument(
        "--host",
    ...
    parser.add_argument(
        "--index", default="documents", type=str, help="Index name to use"
    )
    parser.add_argument("--query", default=None, type=str, help="Search query")
    parser.add_argument(
        "--with_vector", action="store_true", help="Add vector to sample document"
    )
    parser.add_argument(
        "--vector_search", action="store_true", help="Perform vector search demo"
    )
    parser.add_argument(
        "--vector_dim", default=1024, type=int, help="Vector dimension size"
    )
    parser.add_argument(
        "--recreate_index",
        action="store_true",
