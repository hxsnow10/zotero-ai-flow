...
def get_zotero_items(zotero_api_key: str, zotero_library_id: str, 
                     library_type: str = "user", item_types: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    """
    从Zotero库获取文档条目
    ...
    try:
        ...
        with tqdm(total=total_items, desc="获取Zotero条目") as pbar:
            while start < total_items:
                ...
                # 过滤项目类型
                if item_types:
                    batch = [item for item in batch if item['data'].get('itemType') in item_types]
...
def extract_text_from_zotero_item(zot, item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 获取笔记
    try:
        notes = [note for note in zot.children(key) if note['data'].get('itemType') == 'note']
        for note in notes:
...
def prepare_chunk_for_indexing(item_data: Dict[str, Any], chunk: Dict[str, Any], 
                              parent_id: str, chunk_idx: int) -> Dict[str, Any]:
    """
    准备文档切片数据用于ES索引
...
def insert_es(es_client: ElasticsearchClient, item: Dict[str, Any], vector: Optional[List[float]] = None) -> Dict[str, Any]:
    """
    将文档插入Elasticsearch
...
def insert_zotero_item(item, zot, es_client, document_splitter, config, vector_dim, split_enabled, split_method, split_params):
    """
    ...
    try:
        ...
        logger.info(f"split_enabled = {split_enabled}, content = {item_data['content']}, item={item_data}")
        # 处理文档切片
        # 需要考虑切片对象； note需要不需要单独索引
        if split_enabled and (item_data['content'] or item_data['notes']):
            logger.info(f"对文档 '{es_item['title']}' 进行切片...")
            # 切分存在问题
            chunks = sum([document_splitter.split_text(text, method=split_method, **split_params)
                for text in [item_data['content']]
                if len(text)>3000],[])
            logger.info(f"生成了 {len(chunks)} 个切片, conetent={len(item_data['content'])}")
            input("xxxx")
            # 处理每个切片
            with tqdm(total=len(chunks), desc="处理文档切片", leave=False) as chunk_pbar:
...
def main():
    ...
    logger.info(f"文档切片配置: 启用={split_enabled}, 方法={split_method}, 参数={split_params}")
    ...
    items = get_zotero_items(
        ...
        item_types=indexing_config.get("item_types", ["journalArticle", "book", "bookSection", "conferencePaper", "thesis"])
    )
    ...
    logger.info(f"索引完成。成功处理 {success_count}/{len(items)} 个条目。{es_client.index_name} 索引中共有 {count['count']} 个文档。")
    ...
    if split_enabled:
        ...
        parent_count = es_client.es.count(
            index=es_client.index_name,
            body={"query": {"term": {"is_chunk": False}}}
        )
        logger.info(f"其中，主文档 {parent_count['count']} 个，文档切片 {chunk_count['count']} 个")
